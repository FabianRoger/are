...
first, we import the aforementioned file, clean it and have a look at it
...
the sterile control got invaded by (assumingly) environmental bacteria, we included it
as treatment.  
...
each sample as internal standard. The bead solution was sonnicated thoroughly at the beginning
...
As we can see from the corrected bead counts above, the upwards trend 
...
To decide whether to exclude those points, we proceed with an outlier analysis. 
...
Now that the data set is bias corrected and cleaned, we can finally calculate the cell concentration in each sample.
...
cultures experience a long-term trend. As we are only interested in looking how stable the
culture is around the mean, we detrended the stationary phase cell abundance. For that, we fit 
a linear model of the form `Cells/ml ~ date` to the data and calculate the stability on the residuals.
