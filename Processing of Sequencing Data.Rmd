...
from here on we will do the majority of the analysis in usearch
...
We will take out the taxonomy from the OTU table and create a new `TAX` table with the taxonomic assignment of each OTU
...
Both data frames (`TAX_long` & `TAX_wide`) still contain all OTUs. Now, we want to take out all OTUs that are likely to be Chloroplasts or Mitochondria which could have passed the filters. For that we will filter out all OTUs that
...
next with proceed with construction of a phylogenetic tree. before the construction of the tree, we strip the phylogenetic information from the sequence label (otherwise all sequences are named by their full header). Then, we
...
AHH <- function(x) {
  ...
  TFff <- c()
  for (i in 2 : length(x)){
   TF <- c(is.na(x[i-1]), is.na(x[i])) 
   if(identical(TF, c(TRUE, FALSE))) TFff[i-1] <- "AHHHH" else {TFff[i-1] <- "all good"} 
     }
  
  if("AHHHH" %in% TFff) return("AHHHHH") else {return("all good")}
...
EXCL <- c("OTU_74","OTU_132","OTU_153","OTU_181","OTU_190","OTU_220","OTU_245","OTU_267","OTU_444","OTU_483","OTU_489","OTU_515","OTU_564","OTU_572","OTU_573","OTU_600","OTU_611","OTU_629","OTU_637","OTU_634","OTU_686","OTU_732","OTU_774","OTU_793","OTU_924","OTU_978","OTU_999","OTU_1046","OTU_1055")

TAX_long[TAX_long$OTU.ID %in% EXCL&
           TAX_long$tax_order == "d",]
