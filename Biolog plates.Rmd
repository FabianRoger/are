...
ggplot(EcoL_fit, aes(x= estimate))+
  ...
  theme_bw()+
  labs(title = "histogramm of all paramter estimates")
...
EcoL_r <- group_by(EcoL, Sampling, BOT, Wells, rep) %>% 
  ...
  filter(estimate > 0) %>% # instead of replace ODmax < 0 with 0, we exclude them. A carbon source with 0 uptake rate will not be counted as positive
...
EcoL_Func_M <- 
  dcast(EcoL_Func, ID ~ Wells, value.var = "estimate") %T>%
  {assign("RowNames", .$ID, pos = ".GlobalEnv" )} %>%  # we stire the rownames seperately to be sure to not mix up our sample names
  select( -ID) %>% 
  as.matrix()
...
```{r}
diversity(EcoL_Func_M) %>% 
  exp() %>%
  as.data.frame() %>% 
  rename(. ,)
  inner_join(.,EcoL_Func)
